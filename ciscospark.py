...
def get_persondetails(at, personId):
    headers = {'Authorization': _fix_at(at)}
    resp = requests.get(
        _url('/people/{:s}/'.format(personId)), headers=headers)
...
def get_room(at, roomId):
    ...
    resp = requests.get(
        _url('/rooms/{:s}'.format(roomId)), params=payload, headers=headers)
    room_dict = json.loads(resp.text)
    room_dict['statuscode'] = str(resp.status_code)
    return room_dict
...
def get_membership(at, membershipId):
    headers = {'Authorization': _fix_at(at)}
    resp = requests.get(
        _url('/memberships/{:s}'.format(membershipId)), headers=headers)
    membership_dict = json.loads(resp.text)
    membership_dict['statuscode'] = str(resp.status_code)
    return membership_dict
...
def get_message(at, messageId):
    headers = {'Authorization': _fix_at(at)}
    resp = requests.get(
        _url('/messages/{:s}'.format(messageId)), headers=headers)
...
def get_webhook(at, webhookId):
    headers = {'Authorization': _fix_at(at)}
    resp = requests.get(
        _url('/webhooks/{:s}'.format(webhookId)), headers=headers)
    webhook_dict = json.loads(resp.text)
    webhook_dict['statuscode'] = str(resp.status_code)
    return webhook_dict
...
def post_message_rich(at, roomId, text, toPersonId='', toPersonEmail=''):
    headers = {'Authorization': _fix_at(
        at), 'content-type': 'application/json'}
...
def post_localfile(at, roomId, filename, text='', toPersonId='', toPersonEmail=''):
    ...
    resp = requests.request("POST", url=_url(
        '/messages'), data=m, headers=headers)
    file_dict = json.loads(resp.text)
    file_dict['statuscode'] = str(resp.status_code)
    return file_dict
...
def post_membership(at, roomId, personEmail, isModerator=True):
    ...
    resp = requests.post(url=_url('/memberships'),
...
def post_webhook(at, name, targetUrl, resource, event, filter):
    ...
    payload = {'name': name, 'targetUrl': targetUrl,
               'resource': resource, 'event': event, 'filter': filter}
...
def put_room(at, roomId, title='title'):
    ...
    payload = {'title': title}
    resp = requests.put(
        url=_url('/rooms/{:s}'.format(roomId)), json=payload, headers=headers)
    room_dict = json.loads(resp.text)
    room_dict['statuscode'] = str(resp.status_code)
    return room_dict
...
def put_membership(at, membershipId, isModerator):
    ...
    resp = requests.put(url=_url(
        '/memberships/{:s}'.format(membershipId)), json=payload, headers=headers)
    membership_dict = json.loads(resp.text)
    membership_dict['statuscode'] = str(resp.status_code)
    return membership_dict
...
def put_webhook(at, webhookId, name, targetUrl):
    ...
    payload = {'name': name, 'targetUrl': targetUrl}
    resp = requests.put(
        url=_url('/webhooks/{:s}'.format(webhookId)), json=payload, headers=headers)
    webhook_dict = json.loads(resp.text)
    webhook_dict['statuscode'] = str(resp.status_code)
    return webhook_dict
...
def del_room(at, roomId):
    ...
    resp = requests.delete(
        url=_url('/rooms/{:s}'.format(roomId)), headers=headers)
...
def del_membership(at, membershipId):
    headers = {'Authorization': _fix_at(
        at), 'content-type': 'application/json'}
    resp = requests.delete(
        url=_url('/memberships/{:s}'.format(membershipId)), headers=headers)
    del_dict = {'statuscode': str(resp.status_code)}
    return del_dict
...
def del_message(at, messageId):
    headers = {'Authorization': _fix_at(
        at), 'content-type': 'application/json'}
    resp = requests.delete(
        url=_url('/messages/{:s}'.format(messageId)), headers=headers)
    del_dict = {'statuscode': str(resp.status_code)}
    return del_dict
...
def del_webhook(at, webhookId):
    headers = {'Authorization': _fix_at(
        at), 'content-type': 'application/json'}
    resp = requests.delete(
        url=_url('/webhooks/{:s}'.format(webhookId)), headers=headers)
    del_dict = {'statuscode': str(resp.status_code)}
    return del_dict
